...
import thexnator.furnituremod.datastore.DataStoreKey;
import thexnator.furnituremod.datastore.DataStoreManager;
import thexnator.furnituremod.datastore.DataStoreReader;
import thexnator.furnituremod.datastore.DataStoreWrapper;
import thexnator.furnituremod.datastore.DataStoreWriter;

public class IdSyncManager extends DataStoreManager {

	private static final String CHANNEL_NAME = "FurnitureMod|I";
	...
	private class InboundHandler extends SimpleChannelInboundHandler<FMLProxyPacket> {
		...
		public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
			if (evt instanceof NetworkHandshakeEstablished) {
				Log.debug("Sending id data for player: %s", FurnitureMod.proxy.getPlayerFromHandler(((NetworkHandshakeEstablished)evt).netHandler));
				sendAllIds(ctx);
			} else {
