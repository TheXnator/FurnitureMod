...
public class BlockHedge extends BlockFurniture {
	
	public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);

	public BlockHedge(Material materialIn) {
		super(materialIn);
		setHardness(1.0F);
	...
	public void addCollisionBoxesToList(World world, BlockPos pos, IBlockState state, AxisAlignedBB mask, @SuppressWarnings("rawtypes") List list, Entity collidingEntity)
	{
		if (!(collidingEntity instanceof EntitySittableBlock))
		{
			...
			float[] data = CollisionHelper.fixRotation(metadata, 1.0F, 0.7F, 1.5F, 1.0F);
			setBlockBounds(data[0], 1.5F, data[1], data[2], 1.1F, data[3]);
