package thexnator.furnituremod.utils.render;
...
public class GeometryUtils {
	...
	public static void makeLine(int startX, int startY, int startZ, EnumFacing direction, int length, IShapeable shapeable) {
		...
		for (int offset = 0; offset <= length; offset++)
			...
			shapeable.setBlock(startX + (offset * direction.getFrontOffsetX()), startY
					+ (offset * direction.getFrontOffsetY()), startZ
					+ (offset * direction.getFrontOffsetZ()));
	}
	...
	public static void makePlane(int startX, int startY, int startZ, int width, int height, EnumFacing right, EnumFacing up, IShapeable shapeable) {
		...
		for (int h = 0; h <= height; h++) {
			lineOffsetX = startX + (h * up.getFrontOffsetX());
			lineOffsetY = startY + (h * up.getFrontOffsetY());
			lineOffsetZ = startZ + (h * up.getFrontOffsetZ());
